...
def G_wgan_acgan(G,D,  lod, labels, well_facies,  minibatch_size,
    ...
    def Wellpoints_L2loss(well_facies, fake_images): # MSE
        ...
        # loss = tf.nn.l2_loss(well_facies[:,0:1]* (well_facies[:,1:2] - (fake_images+1)/2))
    ...
    def Wellpoints_liklihood_loss(well_facies, fake_images):
        ...
        # loss = tf.nn.l2_loss(well_facies[:,0:1]* (well_facies[:,1:2] - tf.where((fake_images+1)/2>0.4, tf.fill(tf.shape(fake_images), 1.), (fake_images+1)/2)))
        err = well_facies[:,0:1]* (well_facies[:,1:2] - (fake_images+1)/2)
        N_grid = 64*64

        err = tf.transpose(tf.reshape(err,[-1,N_grid]))
        obs_var = (0.1/50)**2 # 归一化的影响
    ...
    def addwellfaciespenalty(well_facies, fake_images_out, loss, Wellfaciesloss_weight):
        with tf.name_scope('WellfaciesPenalty'):
            ...
            # WellfaciesPenalty =  Wellpoints_liklihood_loss(well_facies, fake_images_out)
...
def G_PDE_wgan_acgan(G,  lod, labels, well_facies, velocity, minibatch_size,
    ...
    # grad_v2 = sobel_filter.grad_v(fake_images_out)

    # pde_loss = Ct - grad_hh - grad_vv + grad_h2 +grad_v2
    # pde_loss = tf.nn.l2_loss(  - grad_hh - grad_vv + grad_h2 +grad_v2 )
