...
class TFRecordDataset:
    def __init__(self,
        ...
        # # shuffled List last (t) concentration image
        # tfr_lastconcens_file = tfr_files[-6] 
        # tfr_lastconcens_opt = tf.python_io.TFRecordOptions(tf.python_io.TFRecordCompressionType.NONE)
        # for record in tf.python_io.tf_record_iterator(tfr_lastconcens_file, tfr_lastconcens_opt):
        #     tfr_lastconcens_shape = parse_tfrecord_np_float16(record).shape
        #     break
        ...
        # # unshuffled List last (t) concentration image
        # tfr_lastconcens_refer_file = tfr_files[-4] 
        # tfr_lastconcens_refer_opt = tf.python_io.TFRecordOptions(tf.python_io.TFRecordCompressionType.NONE)
        # for record in tf.python_io.tf_record_iterator(tfr_lastconcens_refer_file, tfr_lastconcens_refer_opt):
        #     tfr_lastconcens_refer_shape = parse_tfrecord_np_float16(record).shape
        #     break
        ...
        # # unshuffled List velocity image
        # tfr_velocity_file = tfr_files[-2] 
        # tfr_velocity_opt = tf.python_io.TFRecordOptions(tf.python_io.TFRecordCompressionType.NONE)
        # for record in tf.python_io.tf_record_iterator(tfr_velocity_file, tfr_velocity_opt):
        #     tfr_velocity_shape = parse_tfrecord_np_float16(record).shape
        #     break

        # # shuffled List label
        # tfr_labels_shuffle_file = tfr_files[-1] 
        # tfr_labels_shuffle_opt = tf.python_io.TFRecordOptions(tf.python_io.TFRecordCompressionType.NONE)
        # for record in tf.python_io.tf_record_iterator(tfr_labels_shuffle_file, tfr_labels_shuffle_opt):
        #     tfr_labels_shuffle_shape = parse_tfrecord_np_float16(record).shape
        #     break
        ...
        with tf.name_scope('Dataset'), tf.device('/cpu:0'):
            ...
            # tf_lastconcens_dset = tf.data.TFRecordDataset(tfr_lastconcens_file, compression_type='', buffer_size=buffer_mb<<17)
            # tf_lastconcens_dset = tf_lastconcens_dset.map(parse_tfrecord_tf_float16, num_parallel_calls=num_threads)             
            # self._tf_lastconcens_dataset = tf_lastconcens_dset
            ...
            # tf_lastconcens_refer_dset = tf.data.TFRecordDataset(tfr_lastconcens_refer_file, compression_type='', buffer_size=buffer_mb<<17)
            # tf_lastconcens_refer_dset = tf_lastconcens_refer_dset.map(parse_tfrecord_tf_float16, num_parallel_calls=num_threads)             
            # self._tf_lastconcens_refer_dataset = tf_lastconcens_refer_dset
            ...
            # tf_velocity_dset = tf.data.TFRecordDataset(tfr_velocity_file, compression_type='', buffer_size=buffer_mb<<17)
            # tf_velocity_dset = tf_velocity_dset.map(parse_tfrecord_tf_float16, num_parallel_calls=num_threads)             
            # self._tf_velocity_dataset = tf_velocity_dset

            # tf_labels_shuffle_dset = tf.data.TFRecordDataset(tfr_labels_shuffle_file, compression_type='', buffer_size=buffer_mb<<17)
            # tf_labels_shuffle_dset = tf_labels_shuffle_dset.map(parse_tfrecord_tf_float16_onevalue, num_parallel_calls=num_threads)             
            # self._tf_labels_shuffle_dataset = tf_labels_shuffle_dset

            self._tf_probimages_wellfacies_dset = tf.data.Dataset.zip( ( self._tf_wellfacies_dataset,self._tf_realrefers_dataset)) 
            # self._tf_probimages_wellfacies_dset = self._tf_wellfacies_dataset
            # self._tf_probimages_wellfacies_dset = tf.data.Dataset.zip(( self._tf_wellfacies_dataset,self._tf_lastconcens_dataset,self._tf_lastconcens_refer_dataset,self._tf_realrefers_dataset,self._tf_velocity_dataset,self._tf_labels_shuffle_dataset))
            if shuffle_mb > 0:
    ...
    def get_minibatch_tf(self, minibatch_size): # => images, labels
        ...
        # wellfacies = self._tf_probimages_wellfacies_iterator.get_next()      
        ...
        # return images, labels,  wellfacies
    ...
    def get_minibatch_well_tf(self, minibatch_size): # => probimages, wellfacies
        ...
        # wellfacies, lastconcens, lastconcensrefers, realrefers, velocity, labels_shuffle = self._tf_probimages_wellfacies_iterator.get_next() 
        if self.well_enlarge:
            # 1) to enlarge area influenced by well facies; 2) since cpu device only accept max_pool opt with data_format = 'NHWC' instead of 'NCHW', so transpose twice to deal with that
            wellfacies = tf.transpose(wellfacies, perm=[0, 2, 3, 1])
            wellfacies = tf.nn.max_pool(wellfacies, ksize = [1,2,2,1], strides=[1,1,1,1], padding='SAME', data_format='NHWC') 
            wellfacies = tf.transpose(wellfacies, perm=[0, 3, 1, 2])          
        return wellfacies, realrefers
        # return  wellfacies, lastconcens, lastconcensrefers, realrefers, velocity, labels_shuffle
