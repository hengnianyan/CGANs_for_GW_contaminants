...
def setup_snapshot_image_grid(G, training_set, size = '6by8'):       # '6by8'=6row and 8 column.
    ...
    wellfacies = np.zeros([gw * gh] + [1,64,64], dtype=np.float32)
    realrefers = np.zeros([gw * gh] + [1,64,64], dtype=np.float32)
    for idx in range(gw * gh):
        x = idx % gw; y = idx // gw
        while True:
            real, label = training_set.get_minibatch_np(1)
            wellface, refer = training_set.get_minibatch_well_np(1)
...
def train_progressive_gan(
    ...
    with tf.device('/gpu:0'):
        if resume_run_id is not None:
            ...
            G = tfutil.Network('G', num_channels=training_set.shape[0], resolution=training_set.shape[1], label_size=training_set.label_size, **config.G)
            D = tfutil.Network('D', num_channels=training_set.shape[0], resolution=training_set.shape[1], label_size=0, **config.D)
            D2 = tfutil.Network('D2', num_channels=training_set.shape[0], resolution=training_set.shape[1], label_size=training_set.label_size, **config.D2) # input two concentration image
            Gs = G.clone('Gs')
    ...
    with tf.name_scope('Inputs'):
        ...
        wellfacies, realrefers = training_set.get_minibatch_well_tf(minibatch_in)
        ...
        # realrefers_split = tf.split(realrefers, config.num_gpus)
    ...
    for gpu in range(config.num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            with tf.name_scope('G_loss'), tf.control_dependencies(lod_assign_ops):
                G_loss = tfutil.call_func_by_name(G=G_gpu, D=D_gpu,  lod = lod_in, labels = labels_gpu, well_facies = wellfacies_gpu, minibatch_size=minibatch_split, G_welllossweight=G_welllossweight_in, **config.G_loss)
            ...
            # with tf.name_scope('G_PDE_loss'), tf.control_dependencies(lod_assign_ops):
            #     [G_PDE_loss,V_loss,Vconsist] = tfutil.call_func_by_name(G=G_gpu, lod = lod_in, labels = labels_gpu, well_facies = wellfacies_gpu, velocity=velocitys_gpu, minibatch_size=minibatch_split, **config.G_PDE_loss)
            

            
            G_opt.register_gradients(tf.reduce_mean(G_loss), G_gpu.trainables)
            # G_PDE_opt.register_gradients(tf.reduce_mean(G_PDE_loss), G_gpu.trainables) # LOSS定义的时候求过平均
            D_opt.register_gradients(tf.reduce_mean(D_loss), D_gpu.trainables)
            D2_opt.register_gradients(tf.reduce_mean(D2_loss), D2_gpu.trainables)
    G_train_op = G_opt.apply_updates()
    # G_PDE_train_op = G_PDE_opt.apply_updates()
    ...
    misc.save_image_grid(grid_realrefers, os.path.join(result_subdir, 'realrefers.png'), drange=[0, 1], grid_size=grid_size)
    
    misc.save_image_grid(grid_fakes[:,0:1,:,:], os.path.join(result_subdir, 'fakes%06d.png' % 0), drange=[-1, 1], grid_size=grid_size)
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        if reset_opt_for_new_lod:
            if np.floor(sched.lod) != np.floor(prev_lod) or np.ceil(sched.lod) != np.ceil(prev_lod):
                ...
                # G_PDE_opt.reset_optimizer_state(); 
        ...
        if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            [G_loss_out] = tfutil.run([tf.reduce_mean(G_loss)], {lod_in: sched.lod, lrate_in: sched.G_lrate, minibatch_in: sched.minibatch,\
                G_pdelossweight_in: sched.G_pdelossweight,G_welllossweight_in: sched.G_welllossweight})
            # [G_PDE_loss_out,V_loss_out,Vconsist_out] = tfutil.run([tf.reduce_mean(G_PDE_loss),tf.reduce_mean(V_loss),tf.reduce_mean(Vconsist)], {lod_in: sched.lod, lrate_in: sched.G_lrate, minibatch_in: sched.minibatch})
            ...
            if cur_tick % image_snapshot_ticks == 0 or done:
                ...
                misc.save_image_grid(grid_fakes[:,0:1,:,:], os.path.join(result_subdir, 'fakes%06d.png' % (cur_nimg // 1000)), drange=drange_net, grid_size=grid_size)
